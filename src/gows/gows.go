...
func (gows *GoWS) SendMessage(ctx context.Context, to types.JID, msg *waE2E.Message, extra whatsmeow.SendRequestExtra) (message *events.Message, err error) {
	...
	if to.User == "status" && to.Server == types.BroadcastServer {
		...
		//
		// [WAHA] A status@broadcast to a large audience can outlast the caller's
		// gRPC deadline. Detach from the request context so a client-side timeout
		// does not abort a send that is already live on WhatsApp — aborting it
		// mid-batch is what previously left later batches undelivered and made the
		// caller believe the status "reached nobody". The send runs to completion
		// and reports exactly who it reached (see SendStatusMessage).
